    ...
    *,
    ...
    if record_queued_event:
        _add_event(
            ...
            {"profile": _ffmpeg_profile(profile, include_bgm=include_bgm, has_audio=False)},
        )
    db.add(run)
    db.commit()
    ...
    try:
        ...
        audio_material = _resolve_audio_material(run, db) if include_bgm else None
        if not materials:
        ...
        for index, shot in enumerate(storyboard, start=1):
            ...
            duration = int(shot.get("duration_seconds") or 3)
            clip_path = clip_dir / f"shot-{index:02d}.mp4"
        ...
        _write_subtitles(subtitle_path, storyboard, profile)
        _run_tool(
            [
                _tool_path("ffmpeg"),
                "-y",
        ...
        if audio_material:
            _mix_audio(video_no_audio_path, audio_material, output_path)
            _add_event(
                db,
                run,
                "audio_mixed",
                ...
                {"audio": _material_payload(audio_material), "tts_status": "placeholder"},
            )
        else:
        ...
        payload = {
            ...
            "source_ai_segments": [
                ...
                if str(item.get("material_id", "")).startswith(("draft:", "replacement:", "seedance:"))
            ],
            "ffmpeg_profile": _ffmpeg_profile(profile, include_bgm=include_bgm, has_audio=bool(audio_material)),
            "has_audio": bool(audio_material),
            "audio_source": _material_payload(audio_material) if audio_material else None,
            "bgm_status": "mixed_from_uploaded_audio" if audio_material else "not_provided",
        ...
        _upsert_artifact(
            ...
            payload={
                ...
                "ffmpeg_profile": _ffmpeg_profile(profile, include_bgm=include_bgm, has_audio=False),
                "has_audio": False,
...
def _missing_ready_draft_segments(run: GenerationRun, draft_artifact: MediaArtifact) -> list[str]:
    ...
    for shot_id, artifact in _replacement_clip_artifacts(run).items():
        ...
        if status == "real_generated" and replacement_payload.get("video_url"):
            continue
        if status in {"real_task_pending", "provider_failed"}:
...
def _materials_from_draft_timeline(run: GenerationRun, draft_artifact: MediaArtifact) -> list[AssemblyMaterial]:
    ...
    for shot in sorted(run.storyboard, key=lambda item: int(item.get("order_index") or 0)):
        ...
        duration = int(shot.get("duration_seconds") or TARGET_DURATION_SECONDS)
        start = cursor
        cursor += duration
    ...
    for shot in sorted(run.storyboard, key=lambda item: int(item.get("order_index") or 0)):
        ...
        materials.append(
            AssemblyMaterial(
                ...
                start_seconds=0,
                end_seconds=int(payload.get("duration_seconds") or shot.get("duration_seconds") or TARGET_DURATION_SECONDS),
            )
        )
    return materials
    ...
    if shot_id:
        match = next(
            (
                ...
                if item.material_id in {f"replacement:{shot_id}", f"draft:{shot_id}", f"seedance:{shot_id}"} or item.usable_for == shot_id
            ),
            None,
        )
        if match:
...
def _mix_audio(video_path: Path, audio_material: AssemblyMaterial, output_path: Path) -> None:
    _run_tool(
        [
            _tool_path("ffmpeg"),
            "-y",
...
def _normalize_storyboard(storyboard: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if not items:
        raise AssemblyError("Storyboard is empty. Run the three agents before assembling video.")
    durations = [max(1, int(item.get("duration_seconds") or 1)) for item in items]
    total = sum(durations)
    if total != TARGET_DURATION_SECONDS:
        durations = _rebalance_durations(durations, TARGET_DURATION_SECONDS)
...
def _ffmpeg_profile(profile: ExportProfile, *, include_bgm: bool, has_audio: bool) -> dict[str, Any]:
    return {
        "renderer": "ffmpeg",
        "width": profile.width,
        ...
        "codec": "libx264",
        "include_bgm": include_bgm,
        "audio": "uploaded_bgm_mixed" if has_audio else "silent_tts_placeholder",
    }
